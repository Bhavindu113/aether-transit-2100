    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 dark:border-slate-800/80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md transition-colors">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between flex-nowrap gap-2 sm:gap-4">
        ...
        <div className="flex items-center flex-shrink-0">
          ...
          >
            <Logo size="md" showTagline={false} className="hidden sm:flex" />
            <Logo size="sm" showTagline={false} className="flex sm:hidden" />
          </button>
        </div>
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 text-xs flex-shrink-0">
          {/* Subtle line status */}
          <NetworkStatus />
          ...
          {/* Voice Assistant - hidden on mobile header since it is in mobile bottom bar and search card */}
          <button
            onClick={() => setVoiceSimOpen(true)}
            className="hidden sm:flex items-center space-x-1.5 px-2.5 py-1.5 rounded-lg text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors font-medium flex-shrink-0"
          ...
          {user ? (
            <div className="flex items-center space-x-1 sm:space-x-2 pl-1.5 sm:pl-2.5 border-l border-slate-200 dark:border-slate-700 flex-shrink-0">
              <div className="flex items-center space-x-1.5 sm:space-x-2 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
                ...
                <span className="text-xs font-bold text-slate-800 dark:text-slate-100 max-w-[55px] xs:max-w-[80px] sm:max-w-[130px] truncate">
                  {user.name.split(' ')[0]}
                </span>
              ...
              <button
                onClick={logout}
                className="p-1.5 sm:px-2.5 sm:py-1.5 rounded-xl text-xs font-semibold text-slate-600 hover:text-rose-600 hover:bg-rose-50 dark:text-slate-300 dark:hover:text-rose-400 dark:hover:bg-rose-950/40 border border-slate-200 dark:border-slate-700 transition-colors flex-shrink-0 flex items-center space-x-1"
                title="Sign Out"
                aria-label="Sign Out"
              >
                <LogOut className="w-3.5 h-3.5 text-rose-500 sm:hidden" />
            ...
            <button
              ...
              className={`flex items-center space-x-1 sm:space-x-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl text-xs font-bold transition-all shadow-sm flex-shrink-0 ${
                activeScreen === 'login'
